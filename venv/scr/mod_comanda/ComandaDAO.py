...
def get_comanda_total(comanda_id: int):
    try:
        ...
        dados = session.query(func.sum(ComandaProdutoDB.quantidade * ComandaProdutoDB.valor_unitario)
                              .label("Total:")).filter(ComandaProdutoDB.comanda_id == comanda_id).scalar()
